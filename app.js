...
app.get('/login/:user', (req, res) => {
  ...
  if (!req.session.createdAt) {
    ...
    req.session.lastAccess = new Date();  // Corregido el nombre de la propiedad

    res.send(`La sesión ha sido iniciada : <h2><strong>${nm}</strong></h2>`);
  } else {
...
app.get('/status', (req, res) => {
  if (req.session.createdAt) {
    ...
    const lastUpdate = new Date(req.session.lastAccess);  // Corregido el nombre de la propiedad
    ...
    res.json({
      ...
      datos_sesion: {
        createdAt: createdAT_MX,
        lastAccess: lastAccess_MX,
        sessionAge: `${hours} horas, ${minutes} minutos y ${seconds} segundos`,
      },
    });
  } else {
  ...
  if (req.session.createdAt) {
    ...
    res.send(`
     <center> <h1>Detalles de la sesión</h1>
      ...
      <p><strong>Último acceso:</strong> ${lastAccess_MX}</p>
      <p><strong>Duración de la sesión (en segundos):</strong> ${sessionDuration}</p></center>
    `);
  } else {
